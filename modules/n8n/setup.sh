#!/usr/bin/env bash
# modules/n8n/setup.sh — pre-deploy hook (run by tuninforge_deploy_module before
...
#      (via tuninforge_get_env) and write them into modules/n8n/.env.
...
: "${TUNINFORGE_MODULES:=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
# shellcheck source=../../lib/log.sh
source "$TUNINFORGE_LIB/log.sh"
# shellcheck source=../../lib/env.sh
source "$TUNINFORGE_LIB/env.sh"

N8N_ENV="$TUNINFORGE_MODULES/n8n/.env"
N8N_DB_NAME="n8n"
...
main() {
  ...
  if [[ "${DRY_RUN:-0}" == "1" ]]; then
    log_info "[dry-run] would read Postgres/Redis passwords via tuninforge_get_env and write them into n8n/.env."
    log_info "[dry-run] would ensure database '$N8N_DB_NAME' + role exist in tuninforge_postgres."
    return 0
  fi

  # 1. Postgres password (must match what Postgres generated).
  local pg_pass pg_user
  pg_pass="$(tuninforge_get_env postgres POSTGRES_PASSWORD || true)"
  pg_user="$(tuninforge_get_env postgres POSTGRES_USER || echo tuninforge)"
  if [[ -z "$pg_pass" ]]; then
  ...
  local redis_pass
  redis_pass="$(tuninforge_get_env redis REDIS_PASSWORD || true)"
  ...
  if ! _dk inspect tuninforge_postgres >/dev/null 2>&1; then
    log_die "tuninforge_postgres is not running; deploy postgres before n8n."
  fi
  ...
  if _dk exec tuninforge_postgres sh -c "psql -U \"$pg_user\" -tAc \"SELECT 1 FROM pg_database WHERE datname='$N8N_DB_NAME'\"" 2>/dev/null | grep -q 1; then
    log_ok "Database '$N8N_DB_NAME' already exists."
  else
    _dk exec tuninforge_postgres sh -c "createdb -U \"$pg_user\" -O \"$pg_user\" \"$N8N_DB_NAME\"" \
