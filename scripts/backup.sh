#!/usr/bin/env bash
# scripts/backup.sh — encrypted backups of the homelab-forge data layer via Restic.
#
# What it backs up:
...
#
# Encryption password:
#   - $FORGE_RESTIC_PASSWORD_FILE (default /var/lib/forge/restic.pass), 0600,
...
STAGING_PARENT="${FORGE_BACKUP_STAGING:-/var/lib/forge/staging}"
...
setup_repo_env() {
  if [[ -n "${RESTIC_REPOSITORY:-}" ]]; then
    ...
    export RESTIC_REPOSITORY="$FORGE_BACKUP_REPO"
  ...
  if [[ "$DRY_RUN" == "1" ]]; then
    log_info "[dry-run] would ensure a password file at $FORGE_RESTIC_PASSWORD_FILE."
    export RESTIC_PASSWORD_FILE="$FORGE_RESTIC_PASSWORD_FILE"
    return 0
  fi

  if [[ ! -f "$FORGE_RESTIC_PASSWORD_FILE" ]]; then
    run_cmd_sudo mkdir -p "$(dirname "$FORGE_RESTIC_PASSWORD_FILE")"
    # Generate a strong passphrase, write 0600 root-owned.
    local pw; pw="$(openssl rand -base64 48 | tr -d '\n')"
    printf '%s\n' "$pw" | run_cmd_sudo tee "$FORGE_RESTIC_PASSWORD_FILE" >/dev/null
    run_cmd_sudo chmod 600 "$FORGE_RESTIC_PASSWORD_FILE"
    log_alert \
      "GENERATED RESTIC ENCRYPTION PASSWORD — SAVE THIS NOW." \
      "File: $FORGE_RESTIC_PASSWORD_FILE (root, 0600)" \
...
# List forge data volumes present on the host.
forge_volumes() {
  _dk volume ls --format '{{.Name}}' 2>/dev/null | grep -E '^forge_.*_data$' || true
}
...
stage_data() {
  ...
  if _dk inspect --format '{{.State.Status}}' forge_postgres >/dev/null 2>&1; then
    ...
    if [[ "$DRY_RUN" == "1" ]]; then
      log_info "[dry-run] would run pg_dumpall inside forge_postgres -> $staging/postgres/all.sql"
    else
      run_cmd_sudo mkdir -p "$staging/postgres"
      # pg_dumpall as the container's POSTGRES_USER.
      _dk exec forge_postgres sh -c 'pg_dumpall -U "$POSTGRES_USER"' \
  ...
  # Every forge_*_data volume -> a tarball in staging via an alpine helper.
  local vol
