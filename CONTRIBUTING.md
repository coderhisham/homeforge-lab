# Contributing to homelab-forge

Thanks for wanting to add to homelab-forge. The project is intentionally
...
homelab-forge is [AGPL-3.0](LICENSE). By contributing you agree your changes are
licensed under it.

## Architecture in one paragraph

`forge.sh` is the entrypoint. `lib/deps.sh` holds the **single-source service
...
Add one pipe-delimited row to `forge_registry()`:
...
- **`docker-compose.yml`**
  ...
  - Join the right external network(s): `forge_public` (Caddy-fronted) and/or
    `forge_internal` (data layer). Both are declared `external: true`.
  ...
  - Prefer a real `healthcheck:` — **but only if the image ships the tool it
    ...
    healthcheck (forge treats "running + stable" as healthy) and do the real
    probe in `healthcheck.sh` (see below).
  - Name volumes `forge_<name>_data` so backup/restore picks them up.
...
- **`.env.example`**
  ...
  - Do **not** put `__GEN__` on a value that must *match* another service's
    ...
    pre-deploy `setup.sh` via `forge_get_env <module> <key>`.
...
- **`healthcheck.sh`** — exit 0 healthy, non-zero not. For services on a network,
  ...
  HTTP endpoint from a throwaway curl container on `forge_internal`, so it does
...
- **`setup.sh`** *(optional)* — a pre-deploy hook run by `forge_deploy_module`
...
- `docker compose -f modules/<name>/docker-compose.yml config` (authoritative
  YAML validation).
- `./forge.sh install --with <name> --dry-run` — confirms deps resolve and the
  deploy sequence is right.
- A real deploy on a **disposable VM** (never a primary box): the service comes
  up, `./modules/<name>/healthcheck.sh` passes, and `./forge.sh remove <name>`
