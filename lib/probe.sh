...
# depend on lib/log.sh or forge's environment. It defines its own Docker
...
[[ -n "${_FORGE_PROBE_SH:-}" ]] && return 0
_FORGE_PROBE_SH=1

# _forge_probe_docker: run docker directly, or via sudo if that's the only way.
_forge_probe_docker() {
...
# forge_http_ready <service> <container> <network> <url>
...
#
# Tunables (env):
#   FORGE_PROBE_ATTEMPTS   number of tries (default 20)
#   FORGE_PROBE_INTERVAL   seconds between tries (default 3)  -> ~60s window
#   FORGE_PROBE_CURL_IMAGE curl image (default curlimages/curl:8.11.1)
forge_http_ready() {
  local service="$1" container="$2" network="$3" url="$4"
  local attempts="${FORGE_PROBE_ATTEMPTS:-20}"
  local interval="${FORGE_PROBE_INTERVAL:-3}"
  local curl_image="${FORGE_PROBE_CURL_IMAGE:-curlimages/curl:8.11.1}"

  # 1. Container must exist and be running.
  local state
  state="$(_forge_probe_docker inspect --format '{{.State.Status}}' "$container" 2>/dev/null || echo missing)"
  ...
  for (( i = 0; i < attempts; i++ )); do
    code="$(_forge_probe_docker run --rm --network "$network" "$curl_image" \
