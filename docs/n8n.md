...
scheduled jobs, build integrations. In homelab-forge it's the first service that
...
1. Reads Postgres's and Redis's generated passwords (via `forge_get_env`) and
...
docker exec forge_postgres psql -U forge -c '\l' | grep n8n     # n8n database exists
```
...
| DB connection refused | Postgres not running / n8n not on `forge_internal`. Check both. |
| "n8n database does not exist" | `setup.sh` couldn't create it. Check `forge_postgres` is healthy; re-run. |
...
Postgres backup — plus the `forge_n8n_data` volume (local files/config). Both are
