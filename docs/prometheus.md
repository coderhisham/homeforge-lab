...
Collects and stores time-series metrics. In tuninforge it ships as three
...
- `tuninforge_internal` only — no published ports, not Caddy-fronted by default.
...
# Targets up? (from a container on tuninforge_internal)
docker run --rm --network tuninforge_internal curlimages/curl:8.11.1 \
  -s http://tuninforge_prometheus:9090/api/v1/targets | grep -o '"health":"[a-z]*"'
```
...
| Symptom | Cause / fix |
|---|---|
| cAdvisor won't start | Needs privileged + `/dev/kmsg`; some kernels differ. Check `docker logs tuninforge_cadvisor`. |
...
`tuninforge_prometheus_data` (the TSDB) is included in `scripts/backup.sh`. Metrics
