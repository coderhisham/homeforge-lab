...
> SSD, watch the `tuninforge_minio_data` volume.

## Isolation & safety

- On **both** networks: `tuninforge_internal` (S3 API for in-cluster services) and
  `tuninforge_public` (Caddy fronts the console over the tailnet). No ports published
  directly.
...
2. **Healthcheck.** Uses `mc ready local` (the `mc` client bundled in the server
   image). If unavailable, tuninforge's liveness fallback still verifies the container
...
docker exec tuninforge_minio mc ready local          # if mc is present
docker logs tuninforge_minio | tail                  # startup + endpoint info
```
...
The `tuninforge_minio_data` volume is archived by `scripts/backup.sh` and restored by
